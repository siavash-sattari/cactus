...
const ProductsList = () => {
  ...
  const productsList = shownProducts.map(prod => (
    <ProductItem key={prod.name} name={prod.name} price={prod.price} img={prod.img} description={prod.description} />
  ));

  return (
    <section className={classes.products}>
      <HeaderProdutos products={filteredProducts} itemsPerPage={itemsPerPage} currPage={currPage} setCurrPage={setCurrPage} />
      <FilterTags />
      {filteredProducts.length ? (
        ...
        <p className={classes.error}>Oops, no results were found for your search.</p>
      )}
    </section>
  );
};
