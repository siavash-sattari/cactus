...
const reducerCart = (state, action) => {
  if (action.type === 'ADD_ITEM') {
    const indexInCart = state.items.findIndex(item => item.name === action.item.name);
  ...
  if (action.type === 'UPDATE_ITEM') {
    const indexItem = state.items.findIndex(item => item.name === action.itemName);
    const oldItem = state.items[indexItem];

    const totalUpdated = state.total - oldItem.amount * oldItem.price + action.amount * state.items[indexItem].price;
  ...
  if (action.type === 'REMOVE_ITEM') {
    ...
    const indexItem = state.items.findIndex(item => item.name === action.name);
